...
import { COMPANY_LOGO_PLACEHOLDER } from '@/lib/constants';
...
export function InternshipCard({ internship }: InternshipCardProps) {
  ...
  const [imgSrc, setImgSrc] = useState(internship.logoUrl || COMPANY_LOGO_PLACEHOLDER);

  useEffect(() => {
    setImgSrc(internship.logoUrl || COMPANY_LOGO_PLACEHOLDER);
  }, [internship.logoUrl]);

  return (
    <article className="bg-white rounded-xl p-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {internship.isActivelyHiring && (
              <Badge className="bg-[#006BFF]/10 text-[#006BFF] border-[#006BFF]/20 text-[11px] font-medium px-2 py-0">
                Actively hiring
              </Badge>
            )}
          </div>
        ...
        <div className="w-12 h-12 rounded  flex items-center justify-center bg-gray-50 flex-shrink-0 overflow-hidden">
          <Image
            src={imgSrc}
            ...
            onError={() => setImgSrc(COMPANY_LOGO_PLACEHOLDER)}
          />
        </div>
      </div>
