...
import { COMPANY_LOGO_BASE_URL, COMPANY_LOGO_PLACEHOLDER } from '@/lib/constants';
...
function buildCompanyLogoUrl(logo: unknown): string | null {
  ...
  return `${COMPANY_LOGO_BASE_URL}/${logo}`;
}
...
export function mapApiResponseToInternships(raw: Record<string, unknown>): Internship[] {
  ...
  return entries.map((item): Internship => {
    ...
    const postedOnDateTime =
      typeof i.postedOnDateTime === 'number' ? i.postedOnDateTime : 0;
    const postedOnTimestamp =
      postedOnDateTime * 1000;
    ...
    return {
      ...
      isActivelyHiring: i.posted_by_label_type === 'success',
      hasJobOffer: i.is_ppo === true,
      isForWomen: i.is_exclusively_for_women === true || i.segment === 'internship_for_women',
      logoUrl: buildCompanyLogoUrl(i.company_logo) ?? COMPANY_LOGO_PLACEHOLDER,
    };
  });
}